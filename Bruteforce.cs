...
namespace SharpUpSQL
{
    class Bruteforce
    {
        ...
        public static void bruteforceSQL(string sqlInstanceParsedName, string sqlInstanceParsedPort)
        {
            SqlConnection cnn;
            foreach (string sqlUsername in Bruteforce.UsernameList)
            {
                foreach (string sqlPassword in Bruteforce.PasswordList)
                {
                    try
                    {
                        cnn = SQLConnect.sqlConnect(sqlInstanceParsedName, sqlInstanceParsedPort, sqlUsername, sqlPassword);
                    }
                    catch (SqlException)
                    {
                        continue;
